...
import { ACESFilmicToneMappingShader, EffectComposer, RGBELoader, ShaderPass } from "three/examples/jsm/Addons.js";
...
export const setupPipeline = async (game: Game) => {
  ...
  composer.addPass(new IBLPass(
    game.scene, game.mainCamera, gBuffer, lightBuffer,
  ...
  composer.passes.forEach((pass) => connectPassToTheatre(pass as RenderPass, game.sheet));
...
const loadEquirect = async (game: Game) => {
  const texture = await new Promise<THREE.Texture>((resolve) => new RGBELoader(game.loadingManager).load('belfast_sunset_puresky_2k.hdr', (texture) => {
    resolve(texture)
  }));
